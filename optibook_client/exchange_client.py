...
class InfoClient(RawClient):
    def __init__(self, host=None, port=None, max_nr_trade_history=100, admin_password=None):
        if not host:
    ...
    def onInstrumentCreated(self, msg):
        i = Instrument.from_json(msg.extraInfo)
    ...
    def poll_new_expired_instruments(self):
...
class ExecClient(Client):
    def __init__(self, host=None, port=None, max_nr_trade_history=100):
        if not host:
            host = _default_settings['host']
        if not port:
    ...
    async def authenticate(self, username=None, password=None, admin_password=None):
        if not username:
